...
export async function watch(inlineConfig) {
    ...
    const logger = inlineConfig.logger ?? new Logger();
    setProcessEnv('development');
    const resolvedUserConfig = await resolveConfig(inlineConfig, 'development', logger, true);
    ...
    if (lazyEnabled) {
        ...
        resolvedUserConfig.compilation.define = {
            ...(resolvedUserConfig.compilation.define ?? {}),
            FARM_NODE_LAZY_COMPILE_SERVER_URL: `http://${hostname.host || 'localhost'}:${resolvedUserConfig.server.port}`
        };
    }
