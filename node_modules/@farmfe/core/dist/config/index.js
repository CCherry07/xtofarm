...
import { Logger, bold, clearScreen, colors, getAliasEntries, green, isArray, isEmptyObject, isObject, isWindows, normalizeBasePath, normalizePath, transformAliasWithVite } from '../utils/index.js';
...
export async function normalizeUserCompilationConfig(resolvedUserConfig, userConfig, logger, mode = 'development', isDefault = false) {
    ...
    const is_entry_html = Object.keys(resolvedCompilation.input).length === 0 ||
        Object.values(resolvedCompilation.input).some((value) => value.endsWith('.html'));
...
async function readConfigFile(inlineOptions, configFilePath, logger) {
    if (fs.existsSync(configFilePath)) {
        ...
        const tsDefaultResolvedUserConfig = await resolveMergedUserConfig(tsDefaultUserConfig, undefined, 'development', logger);
        const normalizedConfig = await normalizeUserCompilationConfig(tsDefaultResolvedUserConfig, tsDefaultUserConfig, logger, 'development');
 ...
 */
export async function loadConfigFile(configPath, inlineOptions, logger = new Logger()) {
    ...
    try {
        ...
        if (configFilePath) {
            const config = await readConfigFile(inlineOptions, configFilePath, logger);
            return {
